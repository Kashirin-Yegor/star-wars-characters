...
export class CharacterStore {
  ...
  public async fetchCharacter(id: number) {
    try {
      ...
      if (characterWithEdits.homeworld) {
        try {
          const planetData = await charactersApi.getPlanet(
            characterWithEdits.homeworld
          );
  ...
  public handleSave() {
    ...
    if (this.editedCharacter.name !== this.character.name)
      changes.name = this.editedCharacter.name;
    if (this.editedCharacter.height !== this.character.height)
      changes.height = this.editedCharacter.height;
    if (this.editedCharacter.mass !== this.character.mass)
      changes.mass = this.editedCharacter.mass;
    if (this.editedCharacter.hair_color !== this.character.hair_color)
      changes.hair_color = this.editedCharacter.hair_color;
    if (this.editedCharacter.skin_color !== this.character.skin_color)
      changes.skin_color = this.editedCharacter.skin_color;
    if (this.editedCharacter.eye_color !== this.character.eye_color)
      changes.eye_color = this.editedCharacter.eye_color;
    if (this.editedCharacter.birth_year !== this.character.birth_year)
      changes.birth_year = this.editedCharacter.birth_year;
    if (this.editedCharacter.gender !== this.character.gender)
      changes.gender = this.editedCharacter.gender;
