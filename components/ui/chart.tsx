    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>
          {labelFormatter(value, payloadArr)}
        </div>
      );
    }
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payloadArr.map((item: Record<string, unknown>, index: number) => {
          ...
          return (
            <div
              key={String((item as { dataKey?: unknown }).dataKey)}
              className={cn(
                  ...
                  >
                    <div className="grid gap-1.5">
                      ...
                      <span className="text-muted-foreground">
                        {itemConfig?.label || String((item as { name?: unknown }).name)}
                      </span>
                    </div>
                    {(item as { value?: unknown }).value != null && (
                      typeof (item as { value?: unknown }).value === "number" ? (
                        <span className="text-foreground font-mono font-medium tabular-nums">
                          {(item as { value: number }).value.toLocaleString()}
                        </span>
                      ) : (
                        <span className="text-foreground font-mono font-medium tabular-nums">
...
function ChartLegendContent(props: ChartLegendContentProps) {
  const { payload = [], verticalAlign = "bottom", className, hideIcon, nameKey } = props;
  const { config } = useChart();
  if (!payload.length) {
    return null;
  }
