...
export function CharacterDetailPage({ characterId }: CharacterDetailPageProps) {
  ...
  const fetchCharacter = async (id: number) => {
    try {
      ...
      // Загружаем информацию о домашней планете
  ...
  const saveLocalEdits = (characterName: string, edits: Partial<Character>) => {
    try {
      const currentEdits = getLocalEdits();
      const updatedEdits = {
        ...currentEdits,
        [characterName]: { ...currentEdits[characterName], ...edits }
      };
  ...
  const handleSave = () => {
    ...
    if (Object.keys(changes).length > 0) {
      saveLocalEdits(character.name, changes);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-4">
            ...
            {/* Цвет кожи */}
            <div>
              <Label htmlFor="skin_color">Цвет кожи</Label>
              {isEditing ? (
                <Input
                  id="skin_color"
            ...
            {/* Фильмы */}
            <div>
              <Label>Появления в фильмах</Label>
